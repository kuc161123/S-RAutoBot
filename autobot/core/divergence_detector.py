...
===============================================
Detects all 4 RSI divergence types.
EMA 200 trend filter is applied at BOS confirmation stage (bot.py), not here.
...
def detect_divergences(df: pd.DataFrame, symbol: str, allowed_types: List[str] = None, lookback_bars: int = LOOKBACK_BARS) -> List[DivergenceSignal]:
    ...
    for i in range(scan_start, n - PIVOT_RIGHT):
        ...
        # [CHANGED] EMA gate moved to BOS confirmation stage in bot.py
        # Divergences are now detected regardless of EMA position.
        # EMA alignment is checked at BOS confirmation time instead,
        # allowing divergences to "queue up" and confirm if price crosses EMA.
        ...
        if curr_pl is not None and prev_pl is not None:
            ...
            # Matches backtest: (i - curr_idx) <= 10
                ...
                trend_aligned = current_price > current_ema  # Record EMA alignment at detection
