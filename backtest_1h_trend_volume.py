#!/usr/bin/env python3
"""
1H REALISTIC BACKTEST - TREND + VOLUME
======================================
Strategy: RSI Divergence + Structure Break
Filters:
1. Volume: break_candle_vol > avg_vol * 1.5
2. Trend: price > EMA 200 for longs, price < EMA 200 for shorts
"""
...
# Settings
DAYS = 365 # 1 year for 1H trend testing
TF = "60"
RR = 3.0
SL_MULT = 1.0 # 1x ATR SL
FEE_PERCENT = 0.0006
...
def calc_indicators(df):
    ...
    df['ema200'] = close_df.ewm(span=200, adjust=False).mean()
...
def run_strategy(df, trend_filter=True, vol_filter=True):
    trades = []
    cooldown = 0
    for i in range(200, len(df)-2):
        ...
        # Trend Filter
        if trend_filter:
            if side == 'long' and row['close'] < row['ema200']: continue
            if side == 'short' and row['close'] > row['ema200']: continue
        ...
        for ahead in range(1, MAX_WAIT_CANDLES + 1):
            ...
            if is_break:
                # Volume Filter
                if vol_filter:
                    if check['vol'] < row['vol_ma'] * 1.5: continue
...
def main():
    symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'XRPUSDT', 'ADAUSDT', 'DOGEUSDT', 'LINKUSDT', 'BCHUSDT', 'AVAXUSDT', 'DOTUSDT', 'PEPEUSDT', 'SHIBUSDT', 'LTCUSDT', 'NEARUSDT', 'SUIUSDT']
    print(f"Testing 1H Divergence + Structure Break with Trend (EMA 200) and Volume Filter...")
    ...
    for tf in [False, True]:
        ...
        for sym in symbols:
            ...
            trades = run_strategy(df, trend_filter=tf, vol_filter=True)
        ...
        if all_trades:
            ...
            print(f"SUMMARY (Trend={tf}): WR={wr:.1f}%, Avg={avg_r:+.3f}R, Total={total_r:+.1f}R (N={len(all_trades)})")
