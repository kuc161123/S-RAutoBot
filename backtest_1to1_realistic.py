...
MIN_TRAIN_TRADES = 10
MIN_TRAIN_WR = 70  # High bar - only combos with 70%+ win rate
...
def simulate_trade_realistic(df, entry_idx, side, entry, atr, max_candles=80):
    ...
    if side == 'long':
        # Entry cost reduces effective entry
...
def run_backtest(num_symbols=400, days=60, train_pct=0.6):
    """Run realistic 1:1 R:R backtest on all symbols."""
    print("="*70)
    print("🔬 REALISTIC 1:1 R:R BACKTEST (400 SYMBOLS)")
    print("="*70)
    ...
    for i, symbol in enumerate(symbols):
        try:
            ...
            if combos and len(combos) > 0:
                ...
                # No winning combos - still show progress
                if total_signals > 0:
                    print(f"[{i+1:3}/{len(symbols)}] {symbol:20} | N={total_signals:4} | WR={overall_wr:5.1f}% | ❌ No 70%+ combos")
                else:
    ...
    # Print results
    print("\n" + "="*70)
    print(f"🏆 1:1 R:R RESULTS (Train WR ≥ {MIN_TRAIN_WR}%, Test WR ≥ {MIN_TEST_WR}%)")
    print("="*70)
    ...
    # Export to YAML
    output_file = 'backtest_1to1_realistic_RESULTS.yaml'
    yaml_output = {
        '_metadata': {
            'rr_ratio': '1:1',
    ...
    with open(output_file, 'w') as f:
        f.write("# REALISTIC 1:1 R:R Backtest Results\n")
