#!/usr/bin/env python3
"""
3M COMPREHENSIVE OPTIMIZATION - Round 3
Proper position tracking with cooldown to match Round 1 methodology

Tests:
1. SL sizes: 0.3%, 0.5%, 0.75%, 1%
2. Trailing configs: 3R, 4R, 5R, 6R targets
3. Filters: No filter, Volume 1x, RSI extreme, Hidden only
4. Cooldowns: 5, 10, 20 bars
"""
...
# Parameters to test
SL_PCTS = [0.003, 0.005, 0.0075, 0.01]  # 0.3%, 0.5%, 0.75%, 1%
MAX_RS = [3, 4, 5, 6]
COOLDOWNS = [5, 10, 20]
FILTERS = ['none', 'volume', 'rsi_extreme', 'hidden']
...
def detect_divergences(df):
    """Detect divergences with metadata"""
    signals = []
    lookback = 20
...
def simulate_trade(df, idx, side, entry, sl_pct, max_r):
    """Simulate single trade with trailing SL"""
...
def run_backtest(symbol_data, sl_pct, max_r, cooldown, filter_type):
    """Run backtest with proper cooldown"""
    total_r = 0
    wins = 0
    trades = 0
    ...
    for symbol, data in symbol_data.items():
        ...
        # Apply filter
        filtered = apply_filter(signals, filter_type)
        
        # Track last trade bar for cooldown
        ...
        for sig in filtered:
            ...
            # Update last trade bar (after trade closes)
...
def main():
    print("=" * 120)
    print("🔬 3M COMPREHENSIVE OPTIMIZATION - ROUND 3")
    print("=" * 120)
    print(f"Timeframe: 3min | Symbols: {SYMBOLS_COUNT} | Days: {DAYS}")
    print(f"SL sizes: {[f'{s*100}%' for s in SL_PCTS]}")
    print(f"Max R targets: {MAX_RS}")
    print(f"Cooldowns: {COOLDOWNS}")
    print(f"Filters: {FILTERS}")
    
    total_configs = len(SL_PCTS) * len(MAX_RS) * len(COOLDOWNS) * len(FILTERS)
    ...
    configs = list(product(SL_PCTS, MAX_RS, COOLDOWNS, FILTERS))
    
    for i, (sl_pct, max_r, cooldown, filter_type) in enumerate(configs):
        if (i + 1) % 20 == 0:
            print(f"  [{i+1}/{len(configs)}] Testing SL={sl_pct*100}% R={max_r} CD={cooldown} F={filter_type}")
        
        result = run_backtest(symbol_data, sl_pct, max_r, cooldown, filter_type)
    ...
    print("=" * 120)
    print(f"{'SL':<8} {'MaxR':<6} {'CD':<5} {'Filter':<12} {'N':>6} {'WR':>7} {'Total_R':>10} {'Avg_R':>10} {'Status':<6}")
    
    for i, row in results_df.head(20).iterrows():
        status = '✅' if row['total_r'] > 0 else '❌'
        print(f"{row['sl_pct']*100:.2f}%  {row['max_r']:<6} {row['cooldown']:<5} {row['filter']:<12} {row['trades']:>6} {row['wr']:>6.1f}% {row['total_r']:>+10.0f} {row['avg_r']:>+10.4f} {status:<6}")
    ...
    # Save results
    results_df.to_csv('3m_round3_results.csv', index=False)
    print("\n✅ Saved to 3m_round3_results.csv")
    ...
    if len(profitable) > 0:
        ...
        print("\n❌ No profitable configuration found in Round 3")
