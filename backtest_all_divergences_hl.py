#!/usr/bin/env python3
"""
ALL DIVERGENCES + HIGHER/LOWER CLOSE CONFIRMATION
==================================================
Entry Filter:
- LONG: Entry candle must close HIGHER than signal candle close
- SHORT: Entry candle must close LOWER than signal candle close

This confirms price continuation in the intended direction.
"""
...
def run():
    print("=" * 70)
    print("📊 ALL DIVERGENCES + HIGHER/LOWER CLOSE")
    print("=" * 70)
    print(f"Entry Filter: LONG close > signal close, SHORT close < signal close")
    ...
    for idx, sym in enumerate(symbols):
        try:
            ...
            for sig in signals:
                ...
                # === HIGHER/LOWER CLOSE CONFIRMATION ===
                signal_row = rows[i]
                entry_row = rows[i + 1] if i + 1 < len(rows) else None
                if not entry_row: continue
                
                if sig['side'] == 'long':
                    # LONG: Entry close must be higher than signal close
                    if entry_row.close <= signal_row.close:
                        filtered_count += 1
                        continue
                else:
                    # SHORT: Entry close must be lower than signal close
                    if entry_row.close >= signal_row.close:
                        filtered_count += 1
                        continue
    ...
    print("BOS:             5285 trades, 57.6% WR, +1.30 EV, +6895R")
    if total_n > 0:
        print(f"Higher/Lower:    {total_n} trades, {combined_wr*100:.1f}% WR, {combined_ev:+.2f} EV, {total_r:+.0f}R")
    print("=" * 70)
