#!/usr/bin/env python3
"""
ALL DIVERGENCES + CANDLE COLOR (FIXED - NO LOOKAHEAD BIAS)
===========================================================
CRITICAL FIX: Checks SIGNAL candle color, not entry candle
- Signal detected on candle i
- Check candle i color (we know this at signal time)
- Enter at candle i+1 open (next candle)

This removes lookahead bias from the original version.
"""
...
def run():
    print("=" * 70)
    print("📊 CANDLE COLOR FILTER - CORRECTED (NO LOOKAHEAD)")
    print("=" * 70)
    print(f"Filter: Check SIGNAL candle (i) color, enter at (i+1) open")
    print(f"LONG: Signal candle must be bullish (close > open)")
    print(f"SHORT: Signal candle must be bearish (close < open)")
    print("=" * 70)
    ...
    for idx, sym in enumerate(symbols):
        try:
            ...
            for sig in signals:
                ...
                # === CANDLE COLOR CHECK (NO LOOKAHEAD) ===
                # Check SIGNAL candle color (we know this at signal time)
                signal_candle_open = row.open
                signal_candle_close = row.close
                
                if sig['side'] == 'long':
                    # LONG: Signal candle must be bullish
                    if signal_candle_close <= signal_candle_open:
                        filtered_count += 1
                        continue
                else:
                    # SHORT: Signal candle must be bearish
                    if signal_candle_close >= signal_candle_open:
                        filtered_count += 1
                        continue
    ...
    print("OLD (Lookahead):       6302 trades, 58.2% WR, +1.33 EV, +8370R ❌")
    if total_n > 0:
        print(f"CORRECTED (Signal):    {total_n} trades, {combined_wr*100:.1f}% WR, {combined_ev:+.2f} EV, {total_r:+.0f}R ✅")
    print("=" * 70)
