#!/usr/bin/env python3
"""
Multi-Combo Backtest (Fixed Timeframe)
Logic: Original Combo Bins (RSI, MACD, VWAP, Fib, MTF)
Features:
- Fixed Timeframe: 5m
- Walk-Forward Validation (Train 70% / Test 30%)
- Filter: WR > 60% (Train AND Test)
- NO Sharpe Filter
- Finds ALL valid combos per symbol
"""
import asyncio
import yaml
import pandas as pd
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("backtest_multi_combo.log"),
...
MIN_TRADES = 20
TRAIN_PCT = 0.7
TIMEFRAME = '5'    # Fixed 5m Timeframe
...
async def backtest_symbol(bybit, sym, idx, total):
    try:
        ...
        for side in ['long', 'short']:
            ...
            # Find ALL valid in train
            valid_train = [r for r in train_results if r['wr'] > WR_THRESHOLD]
            if not valid_train: continue
            
            # Validate in test
            test_results = backtest_fold(test, side)
            
            for train_res in valid_train:
                test_match = next((r for r in test_results if r['combo'] == train_res['combo']), None)
                
                if test_match and test_match['wr'] > WR_THRESHOLD:
                    # Found a winner!
                    res[side].append({
                        'combo': train_res['combo'],
                        'train_wr': train_res['wr'],
                        'test_wr': test_match['wr'],
                        'n': test_match['n']
                    })
                
        if res['long'] or res['short']:
            msg = []
            if res['long']: msg.append(f"LONG: {len(res['long'])} strategies")
            if res['short']: msg.append(f"SHORT: {len(res['short'])} strategies")
...
async def run():
    ...
    # Save Results
    yaml_lines = [
        "# Multi-Combo Optimization Results",
        f"# Logic: Original Combo + Fixed {TIMEFRAME}m",
    ...
    for r in results:
        ...
        if r['long']:
            ...
            for s in r['long']:
                yaml_lines.append(f"    - \"{s['combo']}\"")
                yaml_lines.append(f"      # TrainWR={s['train_wr']:.1f}%, TestWR={s['test_wr']:.1f}%")
        if r['short']:
    ...
    with open('symbol_overrides_MULTI.yaml', 'w') as f:
        f.write("\n".join(yaml_lines))
        
    print(f"✅ Saved to symbol_overrides_MULTI.yaml")
