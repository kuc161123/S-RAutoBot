#!/usr/bin/env python3
"""
EXACT BOT CONFIG TEST - OPTION A (Next Candle Open Entry)
==========================================================
Testing current bot configuration with REALISTIC execution timing:
- Entry: NEXT candle OPEN after structure break (Option A)
- Current config: 5M, lookback=10, wait=10, SL=0.8x, RR=3:1

This tests if the strategy works when executed correctly.
"""
...
# Exact Bot Configuration from config.yaml
DAYS = 200
TIMEFRAME = '5'
SYMBOL_COUNT = 30
RSI_PERIOD = 14
...
def calc_indicators(df):
    ...
    # Swings (using LOOKBACK from config)
...
def run_strategy(df):
    """
    OPTION A: Entry at NEXT candle OPEN after structure break
    """
    trades = []
    ...
    for i in range(50, len(df) - 50):
        ...
        # Step 1: Detect RSI Divergence (ALL types as per config)
        ...
        # Step 2: Wait for Structure Break (max MAX_WAIT candles)
        bos_found = False
        bos_idx = None
        
        for j in range(i+1, min(i+1+MAX_WAIT, len(df)-10)):
            c = df.iloc[j]
            if side == 'long' and c['close'] > row['swing_high']:  # Close above swing high
                bos_found = True
                bos_idx = j
                break
            elif side == 'short' and c['close'] < row['swing_low']:  # Close below swing low
                bos_found = True
                bos_idx = j
                break
        ...
        # Step 3: OPTION A - Enter at NEXT candle OPEN (realistic)
        if bos_idx + 1 >= len(df):
            continue
            
        entry_candle = df.iloc[bos_idx + 1]
        entry = entry_candle['open'] * (1 + (SLIPPAGE if side == 'long' else -SLIPPAGE))
        ...
        # Execute trade
        outcome = None
        for k in range(bos_idx+1, min(bos_idx+1+150, len(df))):
            c = df.iloc[k]
            if side == 'long':
...
def main():
    symbols = get_symbols()
    print("="*80)
    print("EXACT BOT CONFIG TEST - OPTION A (Next Candle Open Entry)")
    print("="*80)
    print(f"Testing {len(symbols)} symbols on 5M timeframe")
    print(f"Config: Lookback={LOOKBACK}, MaxWait={MAX_WAIT}, SL={SL_MULT}x, RR={RR}:1")
    print(f"Entry Timing: NEXT candle OPEN after structure break (realistic)")
    print()
    ...
    print("\n" + "="*80)
    print("RESULTS: EXACT BOT CONFIG WITH OPTION A TIMING")
    print("="*80)
    ...
    if all_trades:
        ...
        if avg_r > 0:
            print("✅ STRATEGY IS PROFITABLE WITH OPTION A TIMING!")
        else:
            print("❌ Strategy shows negative expectancy even with correct timing")
    else:
