#!/usr/bin/env python3
"""
ROBUST HTF COMPARISON
=====================
Tests the "Robust" strategy logic across 5M, 15M, 1H, and 4H.
Shows BOTH Biased (Original) and Realistic (Corrected) results.
"""

import pandas as pd
import numpy as np
import requests
import time
from datetime import datetime

# Robust Parameters
MAX_WAIT_CANDLES = 10
RR = 3.0
SL_MULT = 0.8
FEE_PERCENT = 0.0006
SLIPPAGE_PERCENT = 0.0003

TIMEFRAMES = {
    '5': {'name': '5M', 'days': 60},
    '15': {'name': '15M', 'days': 120},
    '60': {'name': '1H', 'days': 200},
    '240': {'name': '4H', 'days': 300}
}

def get_symbols(n=15):
    try:
        url = "https://api.bybit.com/v5/market/tickers?category=linear"
        resp = requests.get(url, timeout=10).json()
        tickers = resp.get('result', {}).get('list', [])
        usdt = [t for t in tickers if t['symbol'].endswith('USDT')]
        usdt.sort(key=lambda x: float(x.get('turnover24h', 0)), reverse=True)
        BAD = ['XAUTUSDT', 'PAXGUSDT', 'USTCUSDT', 'USDCUSDT', 'BUSDUSDT', 'DAIUSDT']
        return [t['symbol'] for t in usdt[:n] if t['symbol'] not in BAD][:n]
    except:
        return []

def fetch_data(symbol, interval, days):
    try:
        url = "https://api.bybit.com/v5/market/kline"
        all_kline = []
        end_ts = int(time.time() * 1000)
        start_ts = int((time.time() - days * 24 * 3600) * 1000)
        while end_ts > start_ts:
            params = {'category': 'linear', 'symbol': symbol, 'interval': str(interval), 'limit': 1000, 'end': end_ts}
            r = requests.get(url, params=params).json()
            if r['retCode'] != 0 or not r['result']['list']: break
            klines = r['result']['list']
            all_kline.extend(klines)
            end_ts = int(klines[-1][0]) - 1
            time.sleep(0.01)
        if not all_kline: return pd.DataFrame()
        df = pd.DataFrame(all_kline, columns=['ts', 'open', 'high', 'low', 'close', 'vol', 'to'])
        df = df.iloc[::-1].reset_index(drop=True)
        for c in ['open', 'high', 'low', 'close', 'vol']: df[c] = df[c].astype(float)
        return df
    except: return pd.DataFrame()

def calc_indicators(df):
    if len(df) < 50: return pd.DataFrame()
    df = df.copy()
    close = df['close']
    delta = close.diff()
    gain = (delta.where(delta > 0, 0)).rolling(14).mean()
    loss = (-delta.where(delta < 0, 0)).rolling(14).mean()
    rs = gain / (loss + 1e-9)
    df['rsi'] = 100 - (100 / (1 + rs))
    h, l, c_prev = df['high'], df['low'], close.shift(1)
    tr = pd.concat([h-l, (h-c_prev).abs(), (l-c_prev).abs()], axis=1).max(axis=1)
    df['atr'] = tr.rolling(20).mean()
    df['swing_high_10'] = df['high'].rolling(10).max()
    df['swing_low_10'] = df['low'].rolling(10).min()
    df['price_low_14'] = df['low'].rolling(14).min()
    df['price_high_14'] = df['high'].rolling(14).max()
    df['rsi_low_14'] = df['rsi'].rolling(14).min()
    df['rsi_high_14'] = df['rsi'].rolling(14).max()
    df['reg_bull'] = (df['low'] <= df['price_low_14']) & (df['rsi'] > df['rsi_low_14'].shift(14)) & (df['rsi'] < 40)
    df['reg_bear'] = (df['high'] >= df['price_high_14']) & (df['rsi'] < df['rsi_high_14'].shift(14)) & (df['rsi'] > 60)
    return df

def run_strategy(df, realistic=True):
    trades = []
    cooldown = 0
    for i in range(50, len(df)-2):
        if cooldown > 0: cooldown -= 1; continue
        row = df.iloc[i]
        side = 'long' if row['reg_bull'] else 'short' if row['reg_bear'] else None
        if not side: continue
        
        structure_broken, candles_waited = False, 0
        for ahead in range(1, MAX_WAIT_CANDLES + 1):
            if i+ahead >= len(df): break
            check = df.iloc[i+ahead]
            candles_waited = ahead
            if (side == 'long' and check['close'] > row['swing_high_10']) or \
               (side == 'short' and check['close'] < row['swing_low_10']):
                structure_broken = True; break
        
        if not structure_broken: continue
        
        # Biased entry uses the BREAK candle open (look-ahead)
        # Realistic entry uses the NEXT candle open
        idx = i + candles_waited
        entry_idx = idx + 1 if realistic else idx
        
        if entry_idx >= len(df): continue
        
        base = df.iloc[entry_idx]['open']
        entry = base * (1 + SLIPPAGE_PERCENT) if side == 'long' else base * (1 - SLIPPAGE_PERCENT)
        
        atr = row['atr']
        if pd.isna(atr) or atr <= 0: continue
        sl_dist = atr * SL_MULT
        tp_dist = sl_dist * RR
        sl, tp = (entry - sl_dist, entry + tp_dist) if side == 'long' else (entry + sl_dist, entry - tp_dist)
        
        outcome = 'timeout'
        search_start = entry_idx # For biased version, search from same candle? Original used 'idx'
        for j in range(search_start, min(search_start + 300, len(df))):
            c = df.iloc[j]
            # Precise SL/TP check
            if side == 'long':
                if c['low'] <= sl: outcome = 'loss'; break
                if c['high'] >= tp: outcome = 'win'; break
            else:
                if c['high'] >= sl: outcome = 'loss'; break
                if c['low'] <= tp: outcome = 'win'; break
        
        risk_pct = sl_dist / entry
        fee_cost = (FEE_PERCENT + SLIPPAGE_PERCENT) / risk_pct
        res_r = RR - fee_cost if outcome == 'win' else -1.0 - fee_cost if outcome == 'loss' else -0.2
        trades.append({'r': res_r, 'win': outcome == 'win'})
        cooldown = 6
    return trades

def main():
    symbols = get_symbols(15)
    print("=" * 70)
    print("ROBUST HTF COMPARISON: BIASED VS REALISTIC")
    print("=" * 70)
    
    results = []
    
    for tf_code, tf_info in TIMEFRAMES.items():
        print(f"\n--- Testing {tf_info['name']} ({tf_info['days']} days) ---")
        
        # Fetch and prep symbols once
        sym_data = {}
        for sym in symbols:
            df = fetch_data(sym, tf_code, tf_info['days'])
            if len(df) > 100:
                sym_data[sym] = calc_indicators(df)
        
        # Biased Test
        biased_trades = []
        for sym, df in sym_data.items():
            biased_trades.extend(run_strategy(df, realistic=False))
        
        # Realistic Test
        real_trades = []
        for sym, df in sym_data.items():
            real_trades.extend(run_strategy(df, realistic=True))
            
        if real_trades:
            # Biased stats
            b_wr = sum(1 for t in biased_trades if t['win']) / len(biased_trades) * 100 if biased_trades else 0
            b_avg = sum(t['r'] for t in biased_trades) / len(biased_trades) if biased_trades else 0
            
            # Real stats
            r_wr = sum(1 for t in real_trades if t['win']) / len(real_trades) * 100
            r_avg = sum(t['r'] for t in real_trades) / len(real_trades)
            
            print(f"  BIASED:    WR {b_wr:.1f}% | Avg {b_avg:+.3f}R")
            print(f"  REALISTIC: WR {r_wr:.1f}% | Avg {r_avg:+.3f}R")
            
            results.append({
                'tf': tf_info['name'],
                'b_wr': b_wr, 'b_avg': b_avg,
                'r_wr': r_wr, 'r_avg': r_avg,
                'n': len(real_trades)
            })

    print("\n" + "=" * 70)
    print("FINAL SUMMARY COMPARISON")
    print("=" * 70)
    print(f"{'TF':<4} | {'BIASED WR':<10} | {'BIASED R':<10} | {'REAL WR':<10} | {'REAL R':<10}")
    print("-" * 70)
    for res in results:
        print(f"{res['tf']:<4} | {res['b_wr']:9.1f}% | {res['b_avg']:+9.3f}R | {res['r_wr']:9.1f}% | {res['r_avg']:+9.3f}R")
    print("=" * 70)

if __name__ == "__main__":
    main()
