#!/usr/bin/env python3
"""
BASELINE + RSI EXTREMES FILTER
===============================
Only take divergence trades at RSI extremes:
- LONG: RSI < 30 (truly oversold)
- SHORT: RSI > 70 (truly overbought)

Tests if extreme RSI improves divergence reliability.
"""
...
RSI_PERIOD = 14
RSI_EXTREME_LOW = 30   # Must be below this for LONG
RSI_EXTREME_HIGH = 70  # Must be above this for SHORT
LOOKBACK_BARS = 14
    ...
    for i in range(30, n - 5):
        ...
        if curr_pl and prev_pl:
            if curr_pl < prev_pl and rsi[curr_pli] > rsi[prev_pli]:
                signals.append({'idx': i, 'type': 'regular_bullish', 'side': 'long', 'swing': curr_pl, 'rsi': rsi[i]})
                continue
        
        # Regular Bearish
        if curr_ph and prev_ph:
            if curr_ph > prev_ph and rsi[curr_phi] < rsi[prev_phi]:
                signals.append({'idx': i, 'type': 'regular_bearish', 'side': 'short', 'swing': curr_ph, 'rsi': rsi[i]})
                continue
        
        # Hidden Bullish
        if curr_pl and prev_pl:
            if curr_pl > prev_pl and rsi[curr_pli] < rsi[prev_pli]:
                signals.append({'idx': i, 'type': 'hidden_bullish', 'side': 'long', 'swing': curr_pl, 'rsi': rsi[i]})
                continue
        
        # Hidden Bearish
        if curr_ph and prev_ph:
            if curr_ph < prev_ph and rsi[curr_phi] > rsi[prev_phi]:
                signals.append({'idx': i, 'type': 'hidden_bearish', 'side': 'short', 'swing': curr_ph, 'rsi': rsi[i]})
...
def run():
    print("=" * 70)
    print("📊 BASELINE + RSI EXTREMES FILTER")
    print("=" * 70)
    print(f"LONG: RSI < {RSI_EXTREME_LOW} (oversold)")
    print(f"SHORT: RSI > {RSI_EXTREME_HIGH} (overbought)")
    print("=" * 70)
    ...
    for idx, sym in enumerate(symbols):
        try:
            ...
            for sig in signals:
                ...
                signal_rsi = sig['rsi']
                if sig['side'] == 'long':
                    if signal_rsi >= RSI_EXTREME_LOW:
                        filtered_rsi += 1
                        continue
                else:  # short
                    if signal_rsi <= RSI_EXTREME_HIGH:
                        filtered_rsi += 1
                        continue
        ...
        if (idx + 1) % 30 == 0:
            ...
            print(f"  [{idx+1}/{NUM_SYMBOLS}] Trades: {total} | RSI filtered: {filtered_rsi}")
    
    print(f"\n🔍 Filtered by RSI: {filtered_rsi}")
    ...
    if total_n > 0:
        ...
        print(f"   Filtered: {filtered_rsi} ({filtered_rsi/(total_n+filtered_rsi)*100:.1f}%)")
    
    print("\n" + "=" * 70)
    print("📊 COMPARISON")
    print("=" * 70)
    print("Baseline:        6691 trades, 53.2% WR, +1.13 EV, +7553R")
    if total_n > 0:
        print(f"RSI Extremes:    {total_n} trades, {combined_wr*100:.1f}% WR, {combined_ev:+.2f} EV, {total_r:+.0f}R")
        
        if combined_wr > 0.532 and total_r > 7553:
            print("\n✅ RSI filter IMPROVES performance!")
        elif combined_wr > 0.532:
            print("\n⚠️ Higher WR but lower total R (fewer trades)")
        else:
            print("\n❌ RSI filter HURTS performance")
