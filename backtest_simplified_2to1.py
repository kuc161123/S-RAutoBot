...
# R:R RATIO - SAME AS ORIGINAL GOLDEN COMBOS
RR_RATIO = 2.0  # 2:1 R:R

# MINIMUM REQUIREMENTS
MIN_TRAIN_TRADES = 10
MIN_TRAIN_WR = 50  # 50%+ win rate (same as original golden combos criteria)
MIN_TEST_WR = 50   # Must maintain decent performance in test
...
def simulate_trade_realistic(df, entry_idx, side, entry, atr, max_candles=80):
    """
    REALISTIC trade simulation with 2:1 R:R:
    - TP = 2x SL distance
...
def process_single_symbol(symbol: str, days: int = 60, train_pct: float = 0.6) -> dict:
    ...
    # Filter winning combos (WR >= 70% in train, >= 50% in test)
    winning_combos = []
...
def run_backtest(num_symbols=400, days=60, train_pct=0.6):
    """Run realistic 2:1 R:R backtest on all symbols with SIMPLIFIED indicators."""
    print("="*70)
    print("🔬 SIMPLIFIED 2:1 R:R BACKTEST (400 SYMBOLS)")
    print("="*70)
    print(f"   R:R Ratio: {RR_RATIO}:1 (SAME AS ORIGINAL)")
    print(f"   Days: {days}")
    ...
    for i, symbol in enumerate(symbols):
        try:
            ...
            if combos and len(combos) > 0:
                ...
                if total_signals > 0:
                    print(f"[{i+1:3}/{len(symbols)}] {symbol:20} | N={total_signals:4} | WR={overall_wr:5.1f}% | ❌ No {MIN_TRAIN_WR}%+ combos")
                else:
    ...
    # Print results
    print("\n" + "="*70)
    print(f"🏆 2:1 R:R SIMPLIFIED RESULTS (Train WR ≥ {MIN_TRAIN_WR}%, Test WR ≥ {MIN_TEST_WR}%)")
    print("="*70)
    ...
    # Export to YAML
    output_file = 'backtest_simplified_2to1_RESULTS.yaml'
    yaml_output = {
        '_metadata': {
            'rr_ratio': '2:1',
    ...
    with open(output_file, 'w') as f:
        f.write("# SIMPLIFIED 2:1 R:R Backtest Results\n")
        f.write(f"# Generated: {datetime.utcnow().isoformat()}\n")
        f.write(f"# R:R Ratio: 2:1 (same as original)\n")
