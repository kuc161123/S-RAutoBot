#!/usr/bin/env python3
"""
SIMPLIFIED 2:1 R:R Backtest - HIGH SAMPLE COUNT, 40% WR THRESHOLD

Same simplified bins as current bot (3 RSI x 2 MACD x 3 Fib = 18 combos)
With higher sample requirements and 40% WR threshold.

Features:
- Walk-forward validation (60% train, 40% test)
- Realistic fees
- 2:1 R:R (same as current bot)
- Simplified 18 indicator bins
- Higher sample counts: 20 train / 5 test minimum
- 40% WR threshold for both train and test
- Results saved to separate file
...
# R:R RATIO - 2:1 (same as current bot)
RR_RATIO = 2.0

# MINIMUM REQUIREMENTS - HIGHER SAMPLE COUNTS
MIN_TRAIN_TRADES = 20    # Need at least 20 trades in training
MIN_TEST_TRADES = 5      # Need at least 5 trades in test
MIN_TRAIN_WR = 40        # 40%+ win rate in training
MIN_TEST_WR = 40         # 40%+ win rate in test
...
def get_combo_simplified(row) -> str:
    """
    SIMPLIFIED combo bins - same as current bot:
...
def run_backtest(num_symbols=400, days=60, train_pct=0.6):
    print("="*70)
    print("🔬 SIMPLIFIED 2:1 R:R BACKTEST - HIGH SAMPLE COUNT, 40% WR")
    print("="*70)
    print(f"   R:R Ratio: {RR_RATIO}:1 (same as current bot)")
    print(f"   Days: {days}")
    ...
    print(f"   Indicator Bins: SIMPLIFIED (3 RSI x 2 MACD x 3 Fib = 18 combos)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print(f"🏆 SIMPLIFIED 2:1 R:R RESULTS (High Sample Count, 40% WR)")
    ...
    # SEPARATE OUTPUT FILE - does not affect bot
    output_file = 'backtest_simplified_2to1_HIGH_SAMPLE_40WR_RESULTS.yaml'
    yaml_output = {
        '_metadata': {
            'rr_ratio': '2:1',
    ...
    with open(output_file, 'w') as f:
        f.write("# SIMPLIFIED 2:1 R:R Backtest Results (HIGH SAMPLE COUNT, 40% WR)\n")
        f.write(f"# Generated: {datetime.utcnow().isoformat()}\n")
        f.write(f"# R:R Ratio: 2:1 (same as current bot)\n")
