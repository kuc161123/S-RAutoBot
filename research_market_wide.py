#!/usr/bin/env python3
"""
MARKET-WIDE PRECISION SEARCH (All Symbols)
==========================================
Goal: Exhaustive search of EVERY USDT pair on Bybit to find "Unicorn" configurations.

Methodology:
1.  Fetch ALL active USDT Linear Perpetuals from Bybit API.
2.  Precision Engine (1H Signal / 5M Execution) - Deduped.
3.  Grid Search per Symbol.
4.  Robustness Filter (Min 10 trades, >0.1R expectancy).

Warning: This script is resource intensive and takes hours.
Output: market_wide_results.csv
"""
...
OUTPUT_FILE = 'market_wide_results.csv'
    ...
    for div_type, atr, rr in combos:
        ...
        if passed:
            ...
            with FILE_LOCK:
                ...
                pd.DataFrame([res_dict]).to_csv(OUTPUT_FILE, mode='a', header=header, index=False)
                
    return best_results

def main():
    print(f"🚀 MARKET-WIDE PRECISION SEARCH (ALL SYMBOLS)")
    target_symbols = fetch_all_symbols()
    ...
    completed = 0
    # Increased threads slightly as we are hitting I/O limits
    with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
        futures = {executor.submit(optimize_symbol, sym): sym for sym in target_symbols}
        for future in concurrent.futures.as_completed(futures):
            completed += 1
            # Simple progress bar
            pct = (completed / len(target_symbols)) * 100
            sys.stdout.write(f"\rProgress: {completed}/{len(target_symbols)} ({pct:.1f}%)")
            sys.stdout.flush()
